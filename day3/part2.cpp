...
int main() {
    ...
    for(int i = 0; i < input_data.size(); i++){
        ...
        for(int j = 0; j < input_data[i].size(); j++){
            if(input_data[i].substr(j, 4) == "do()") enabled = true;
            else if(input_data[i].substr(j, 7) == "don't()") enabled = false;
            else if(enabled && input_data[i].substr(j, 4) == "mul(") result += Get_Num(input_data[i], j);
        }
    }
